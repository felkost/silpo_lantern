...
export interface Candidate {
  ...
  /** "add" is the ordinary consent screen; "compensate" is the
  ...
  /** computed server-side with the guard's own canonicalizer. */
  args_hash: string;
  tool_name: string;
...
export interface ConsentAckResponse {
  status: string;
  action_id: string;
  /** the binding as recorded server-side. No cart id, ever. */
  args_hash: string;
  state_hash: string;
  expires_at: string;
}

/** one evidence tuple behind a candidate -- price, its source and its
...
/** `GET /evidence` -- measured earlier, never this session. */
...
export interface DisclosedValidation {
  ...
  type: string;
  /** whether the policy registry knows this code. */
  is_known?: boolean;
}
...
export interface DiagnosisEvent extends EventEnvelope {
  ...
  /** the arithmetic's inputs, as decimal strings. */
...
export interface ReceiptEvent extends EventEnvelope {
  status: string;
  reason: string | null;
  /** expected against actual, and the typed outcome. */
  ...
  cart: CartView | null;
  /** a verified write is not a recovered cart -- distinguishes
