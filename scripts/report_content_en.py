...
<p>The guest talks only to the interface — one FastAPI service that serves the API and
the built React console from the same origin. The interface starts the Recovery Agent, a
LangGraph graph. The agent uses the Domain Core to compute every number, and it must
...
<a href="safety.html#guard">safety page</a>. Only the MCP Adapter talks to Silpo; the
guest's browser can never reach the Silpo server directly.</p>
...
called</td></tr>
<tr><td>Infra</td><td>MCP client and OAuth, dynamic tool registry with quarantine, Neon
...
<p>The brief names thirteen rules about money, time and evidence. Each one exists here
as its own test file — <code>tests/unit/test_dr_01…13</code> — named after the rule,
so "the rule holds" is something the test suite answers rather than a claim in a
document. None of them needs a network to run.</p>
<table>
...
<p>The tool list is discovered live through <code>tools/list</code>, cached with an
