"""T3 (G8 stage spec): `reviewed_tools.json`'s `source`, `source_schema_hash`
and `reviewed_at` must all describe the SAME baseline as its stored
`tool_hashes`. Found stale (audit row 10, G8 kickoff): `source` named the
...
2026-09-07 fixture (D47's live regeneration) -- a file that misdescribes
